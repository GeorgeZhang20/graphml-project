...
|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|----------|
| **George**  | E2 + all of E2's prerequisites (Goodreads build, `load_goodreads_children.py`, registry edit, `llm_explanations/generate.py`, both DeBERTa fine-tunes × 4 seeds, GNN sweep, aggregation, plotting harness). Plus E3 + A5: a single frozen-DeBERTa script that produces baseline `.emb` files for raw text (E3) and explanation text (A5), then re-runs the GNN sweep on those. | **E2, E3, A5** | Haoran |
| **Haoran**  | ogbn-arxiv replication harness, downloading the authors' cached `gpt_responses` + `.emb` checkpoints, running E1; plus the drop-one ensemble ablation (A1) on Goodreads once George's TA/E `.emb` files are pushed | E1, A1      | George   |
| **Yiwen**   | Top-k sweep for `P` (small patch to make `topk` configurable in `gnn_trainer.py`, then sweep k ∈ {1, 3, 5}); low-label regime A6 (subsamples `train_mask` to {25%, 50%, 100%} and re-runs the GNN sweep on existing `.emb` files) | A2, A6      | Haoran   |
| **unassigned** | A3 (prompt template), A4 (Llama backend), E4 (Cora sanity) — pick up if a phase finishes early; drop if not | — | — |
